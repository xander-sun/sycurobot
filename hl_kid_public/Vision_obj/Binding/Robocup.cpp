...
using namespace Vision_obj::Localisation;
...
using Vision_obj::Utils::ImageLogger;
using Vision_obj::Filters::BallProvider;
using Vision_obj::Filters::CompassProvider;
using Vision_obj::Filters::TagsDetector;
...
void Robocup::initRhIO() {
  ...
  if (RhIO::Root.commandExist("Vision_obj/logLocal")) {
    return;
  }
  RhIO::Root.newStr("/Vision_obj/cameraStatus")->defaultValue("");
  RhIO::Root.newFloat("/Vision_obj/lastUpdate")
      ->defaultValue(-1)
  ...
  if (isFakeMode()) {/// Highgui is not available on robot
    RhIO::Root.newCommand(
      "Vision_obj/showFilters", "Display the given filters",
    ...
    RhIO::Root.newCommand(
      "Vision_obj/hideFilters", "Hide the given filters",
      [this](const std::vector<std::string> &args) -> std::string {
        if (args.size() < 1) {
  ...
  RhIO::Root.newCommand(
      "Vision_obj/logLocal", "Starts logging for a specified duration. Images are "
  ...
  for (const SpecialImageHandler &sih : imageHandlers) {
    std::string prefix = "Vision_obj/" + sih.name;
...
void Robocup::step() {
  ...
  importFromRhIO();
  Benchmark::open("Vision_obj + Localisation");
  ...
  try {
    Application::step();
    activeSource = true;
    // If Vision_obj application has finished, ask for scheduler to shut down
    if (!isActive()) {
      out.log("Vision_obj exiting, asking to scheduler to shut down");
      _scheduler->askQuit();
    }
  } catch (const Vision_obj::Filters::PtGreyException &exc) {
    globalMutex.unlock();
    Benchmark::close("Pipeline");
    Benchmark::close("Vision_obj + Localisation", benchmark, benchmarkDetail);
    activeSource = false;
    ...
    return;
  } catch (const Vision_obj::Filters::PtGreyConnectionException &exc) {
    globalMutex.unlock();
    Benchmark::close("Pipeline");
    Benchmark::close("Vision_obj + Localisation", benchmark, benchmarkDetail);
    activeSource = false;
...
void Robocup::importFromRhIO() {
  autologMovingBall = RhIO::Root.getValueBool("/Vision_obj/autologMovingBall").value;
  autolog_games = RhIO::Root.getValueBool("/Vision_obj/autologGames").value;
  logBallExtraTime = RhIO::Root.getValueFloat("/Vision_obj/logBallExtraTime").value;
  benchmark = RhIO::Root.getValueBool("/Vision_obj/benchmark").value;
  benchmarkDetail = RhIO::Root.getValueInt("/Vision_obj/benchmarkDetail").value;
  imageDelay = RhIO::Root.getValueInt("/Vision_obj/imageDelay").value;
  angularPitchTolerance =
      RhIO::Root.getValueFloat("/Vision_obj/angularPitchTolerance").value;
...
void Robocup::publishToRhIO() {
  RhIO::Root.setFloat("/Vision_obj/lastUpdate", diffMs(lastTS, getNowTS()));
  std::string cameraStatus = getCameraStatus();
  RhIO::Root.setStr("/Vision_obj/cameraStatus", cameraStatus);
}
...
void Robocup::readPipeline() {
  ...
  // Goals:
  goalsMutex.lock();
  for (const std::string &provider_name : ballProviders) {
    try {
      const Filters::BallProvider &provider =
          dynamic_cast<const Filters::BallProvider &>(
              pipeline.get(provider_name));
      std::vector<cv::Rect> providerGoals = provider.getGoals(); 
      
      // Converting the goal from image basis to "origin" basis
      // and adding it to detected goals
      for (size_t id = 0; id < providerGoals.size(); id++) {
        cv::Rect goal_rect = providerGoals[id];
        int providerGoalsX = goal_rect.x + cvRound(goal_rect.width/2.0);
        int providerGoalsY = goal_rect.y + cvRound(goal_rect.height/2.0);
        //std::vector<double> providerGoalsY = provider.getGoalsY();
        double goal_x = providerGoalsX;
        double goal_y = providerGoalsY;
      ...
      std::cerr << "Robocup::readPipeline: goalProviders (changed to ballProviders): runtime_error: "
                << exc.what() << std::endl;
    }
  }
  goalsMutex.unlock();
...
*/
  ...
  for (const std::string &providerName : ballProviders) {
    try {
      ...
      Vision_obj::Filter &ballFilter = pipeline.get(providerName);
      const BallProvider &provider =
          dynamic_cast<const BallProvider &>(ballFilter);
      std::vector<cv::Rect> Balls = provider.getBalls();
      std::vector<double> ballsX;
      std::vector<double> ballsY;
      std::vector<double> ballsRadius;
      for (size_t id = 0; id < Balls.size(); id++)
      {
        cv::Rect ball_rect = Balls[id];
        int ballx = ball_rect.x + cvRound(ball_rect.width/2.0);
        int bally = ball_rect.y + cvRound(ball_rect.height/2.0);
        ballsX.push_back(ballx);
        ballsY.push_back(bally);
        ballsRadius.push_back(cvRound(ball_rect.width/2.0));
      }
...
/*
  ...
  if (pipeline.isFilterPresent("obstacleByDNN")) {
    try {
      Vision_obj::Filter &robotDetector_f = pipeline.get("obstacleByDNN");
...
*/
  ...
  for (const std::string & tagProviderName : tagProviders) {
    Vision_obj::Filter &tagFilter = pipeline.get(tagProviderName);
  ...
  for (const std::string & name : fieldBorderProviders) {
    Vision_obj::Filter & fieldBorder_f = pipeline.get(name);
    try {
  ...
  */
  ...
  if (pipeline.isFilterPresent("fieldBorder")) {
    Vision_obj::Filter & fieldBorder_f = pipeline.get("fieldBorder");
    try {
      const Filters::FieldBorder & fieldBorder =
        dynamic_cast<const Filters::FieldBorder &>(fieldBorder_f);
      Filters::FieldBorderData data = fieldBorder.loc_data;
  ...
  for (const std::string & name : compassProviders) {
    Vision_obj::Filter & compassFilter = pipeline.get(name);
    try {
      const Vision_obj::Filters::VisualCompass & CompassProvider =
        dynamic_cast<const Vision_obj::Filters::VisualCompass &>(compassFilter);
